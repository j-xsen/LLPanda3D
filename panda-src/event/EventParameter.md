...
copy (it's just a smart pointer). Transparently wraps the common cases —
int, double, `std::string`, `std::wstring`, or a
`TypedReferenceCount`/`TypedWritableReferenceCount` pointer — via the
`ParamValue<T>` template family from `paramValue.h`, so you rarely need to
know the wrapping exists.

## Behavior notes

- **Construct implicitly from any supported type** — the constructors are all
  non-`explicit`, so `EventParameter(42)`, `EventParameter("hi")`,
  `EventParameter(some_node_ptr)` all just work, and `throw_event(name, 42)`
  works directly without wrapping.
- **`EventParameter(TypedReferenceCount*)` vs. `EventParameter(TypedWritableReferenceCount*)`
  ...
  stored directly. Check with `is_typed_ref_count()` vs. just calling `get_ptr()`
  and inspecting its `TypeHandle` accordingly.
...
- **`get_ptr()`** is the escape hatch for parameter types that aren't one of
  ...
  the raw `TypedWritableReferenceCount*`, and you inspect `get_type()` /
  `is_of_type()` / cast (`DCAST`) yourself.
