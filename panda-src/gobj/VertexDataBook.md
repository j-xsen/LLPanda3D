...
- `save_to_disk()` at the book level calls `save_to_disk()` on every contained page directly — a bulk "flush everything to disk now" operation, e.g. useful before an expected memory-pressure event or at shutdown.
